...
def vae_loss(recon_x, x, mu, logvar, beta=1.0):
    ...
    """
    # Ensure reconstructed and target tensors have the same spatial size
    if recon_x.shape != x.shape:
        # Resize reconstructed output to match target spatial dimensions (assumes NCHW)
        try:
            recon_x = F.interpolate(recon_x, size=x.shape[2:], mode='bilinear', align_corners=False)
        except Exception:
            recon_x = F.interpolate(recon_x, size=x.shape[2:], mode='nearest')
        import warnings
        warnings.warn(f"Resized recon_x to match target shape for loss: {recon_x.shape} vs {x.shape}")
